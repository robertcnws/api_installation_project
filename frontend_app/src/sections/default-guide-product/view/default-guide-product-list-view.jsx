...
import { DefaultGuideProductTableRow } from '../default-guide-product-table-row';
import { DefaultGuideProductTableToolbar } from '../default-guide-product-table-toolbar';
import { DefaultGuideProductTableFiltersResult } from '../default-guide-product-table-filters-result';

// ----------------------------------------------------------------------

const STATUS_OPTIONS = [{ value: 'all', label: 'All Default Guide Products' }].concat([
...
export function DefaultGuideProductListView() {

  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedDefaultGuideProducts, loadingDefaultGuideProducts, errorDefaultGuideProducts, refetchDefaultGuideProducts } = useDataContext();
  ...
  const filters = useSetState({ name: '', status: localStorage.getItem('defaultGuideProductStatus') || 'all' });
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentDefaultGuideProductId');
  }, []);
  ...
  useEffect(() => {
    if (refetchDefaultGuideProducts) {
      refetchDefaultGuideProducts();
    }
    setTableData(loadedDefaultGuideProducts || []);
  }, [refetchDefaultGuideProducts, loadedDefaultGuideProducts]);

  useEffect(() => {
    if (loadedDefaultGuideProducts) {
      setTableData(loadedDefaultGuideProducts);
    }
  }, [loadedDefaultGuideProducts]);

  useEffect(() => {
    const socket = new WebSocket(`${CONFIG.wsProtocol}://${CONFIG.wsHost}/${CONFIG.wsDomain}/projects/ws/default-guide-products/`);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await axios.delete(`${CONFIG.apiUrl}/projects/delete/default-guide-product/${id}/`, {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await axios.delete(`${CONFIG.apiUrl}/projects/delete/default-guide-products/`, {
        data: {
          ids: table.selected,
          userReporter: userLogged?.data,
        },
  ...
  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentDefaultGuideProductId', id);
      router.push(paths.dashboard.defaultGuideProduct.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      ...
      localStorage.removeItem('routeByShipment');
      localStorage.removeItem('routeByShipmentBySku');
      localStorage.setItem('defaultGuideProductStatus', filters.state.status);
      router.push(paths.dashboard.defaultGuideProduct.details(id));
    },
    [router, filters]
  );

  if (loadingDefaultGuideProducts) {
    return (
      <DashboardContent>
  ...
  if (errorDefaultGuideProducts) {
    return (
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorDefaultGuideProducts.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Default Guide Product', href: paths.dashboard.defaultGuideProduct.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.defaultGuideProduct.new}
              // color="inherit"
              // variant="outlined"
              variant="contained"
              startIcon={<Iconify icon="mingcute:add-line" />}
            >
              New Default Guide Product
            </Button>
          }
          sx={{ mb: { xs: 3, md: 5 } }}
        />
        ...
        <Card>
          ...
          <DefaultGuideProductTableToolbar
            filters={filters}
            ...
            title={filters.state.status === 'all' ? 'All Default Guide Products' :
              filters.state.status === 'active' ? 'Active Default Guide Products' :
                filters.state.status === 'inactive' ? 'Inactive Default Guide Products' :
                  filters.state.status
            }
          />

          {canReset && (
            <DefaultGuideProductTableFiltersResult
              filters={filters}
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> default guide products?
          </>
        }
        action={
          <Button
