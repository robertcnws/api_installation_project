...
export function DefaultGuideProductTableRow({ row, selected, onEditRow, onSelectRow, onDeleteRow, onViewRow, onReturnList }) {
  ...
  return (
    <>
      <TableRow hover selected={selected} aria-checked={selected} tabIndex={-1} sx={{ cursor: 'pointer' }}>
        ...
        {!isMobile ? (
          <>
            ...
            <TableCell sx={{ whiteSpace: 'nowrap' }} onClick={() => onEditRow()}>{fCurrency(row.price)}</TableCell>
      ...
      <ConfirmDialog
        ...
        title="Delete"
        content={`Are you sure want to delete guide product: (${row.name})?`}
        action={
