...
export function AnalyticsMetricsServiceSummary({
  ...
  const sortedTasks = useMemo(() => {
    const serviceDefaultTasks =
      allServices && allServices[0] && Array.isArray(allServices[0].serviceDefaultTasks)
        ? allServices[0].serviceDefaultTasks
        : [];
    const tasks = [...serviceDefaultTasks];

    return tasks.sort((a, b) => {
      const orderA = a?.service_default_task?.order ?? 0;
      const orderB = b?.service_default_task?.order ?? 0;
  ...
  const arrayDates = useMemo(() => {
    if (!Array.isArray(allServices) || allServices.length === 0 || !firstTask || !lastTask) {
      return [];
    }

    return allServices
      .map((service) => {
        const tasks = service.serviceDefaultTasks ?? [];

        const startTask = tasks.find(
          (task) =>
            task.service_default_task?.id === firstTask.service_default_task?.id &&
            task?.status === 'finished'
        );

        const endTask = tasks.find(
          (task) =>
            task.service_default_task?.id === lastTask.service_default_task?.id &&
            task?.status === 'finished'
        );
      ...
      .filter(Boolean);
  }, [allServices, firstTask, lastTask]);

  const {
    averageDuration,
    minDuration,
    maxDuration,
    minProjectId: minServiceId,
    maxProjectId: maxServiceId,
    minProjectName: minServiceName,
    maxProjectName: maxServiceName,
    minProjectNumber: minServiceNumber,
    maxProjectNumber: maxServiceNumber,
  } = useMemo(() => fDurationStats(arrayDates), [arrayDates]);

  const finishedCount = finishedservices?.length || 0;
