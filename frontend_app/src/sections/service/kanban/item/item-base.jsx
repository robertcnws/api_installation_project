...
const ItemBase = forwardRef(({
  ...
  const extraTasks = useMemo(() => service?.serviceDefaultTasks.filter(
    t => tasksBeforeNoMatter.some(item => item.toLowerCase().includes(t.service_default_task.name.toLowerCase()))
  ), [service, tasksBeforeNoMatter]);
