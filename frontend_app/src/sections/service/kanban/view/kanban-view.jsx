...
export function KanbanView({
  ...
  useEffect(() => {
    if (service) {
      setHasInstallDate(service.startDate !== null);
    }
  }, [service]);
...
function adaptToKanbanData(serviceTasks, stages) {
  const columns = [];
  const tasks = {};
  ...
  serviceTasks?.forEach((task) => {
    const colId = task.service_default_task.service_stage.id;
    if (colId && tasks[colId]) {
      tasks[colId].push({
        ...task,
        id: task.service_default_task.id,
        name: task.service_default_task.name,
        description: task.service_default_task.description,
