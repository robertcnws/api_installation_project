...
export function ServiceAttachmentsTableRow({
  row,
  selected,
  onSelectRow,
  onDeleteRow,
  ...
  // const { refetch: refetchService } = useServiceByIdQuery(row?.id, {
  //   skip: !row?.id,
  // });
  ...
  //     refetchService()
  //       .then((response) => {
  //         setRowUpdated(response.data.serviceById);
  //       })
  //       .catch((error) => {
    ...
    <>
      ...
      >
        ...
        <TableCell
          // onClick={handleClick} 
          onClick={() => {
            localStorage.removeItem('serviceReminderTab');
        ...
        {loadedServiceStages.map((stage) => {
          ...
          const attachments = getServiceAttachments(rowUpdated, stageName);
                      ...
                      <Box sx={{
                        ...
                        {attachments?.service?.length > 0 && (
                          <Typography variant='body2' sx={{ color: 'text.secondary', fontSize: '0.75rem', textAlign: 'center' }}>
                            {`${attachments?.service?.length} Files`}
                          </Typography>
                        )}
                      </Box>
                    </Box>
