...
export function ServiceEditAttachments({
  service,
  refetchService,
  id,
  name,
  ...
  const mappedDisplayFiles = useMemo(() => {
    if (!loadedServiceStages || !service || !service.currentStage) return [];
    return loadedServiceStages?.map((stage) => {
      const filesForStage = displayFiles.filter((file) =>
        file.isNew ? service.currentStage.id === stage.id : file.current_stage && file.current_stage.id === stage.id
      );
      return {
  ...
  }, [displayFiles, loadedServiceStages, service]);
  ...
  const handleConfirmRemove = async () => {
    ...
    if (!fileToRemove.isNew && !isLocalFile) {
      try {
        await axios.delete(`${CONFIG.apiUrl}/services/delete/file/${id}/service/${fileToRemove.file}/`, {
          data: {
  ...
  const handleAddFiles = async () => {
    ...
    try {
      ...
      await axios.post(`${CONFIG.apiUrl}/services/upload/service/${id}/file/`, formData, {
        headers: {
    ...
    <>
      <Box sx={{ maxHeight: 600, minHeight: !isMobile ? 600 : 0, overflow: 'auto' }}>
        {(service?.currentStage &&
          service?.currentStage?.name.toLowerCase().indexOf(CONFIG.stages.finished.toLowerCase()) === -1) && (
            <>
              <Box
                ...
                <Grid container spacing={2}>
                  <Grid item xs={4} sm={2}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                      <Box sx={{ mb: 1, typography: 'overline' }}>
                        <b>{service?.currentStage?.name}</b>
                      </Box>
                      {(verifyPermissions(
                        listPermissions,
                        CONFIG.permissions.system,
                        CONFIG.permissions.moduleServices,
                  ...
                  <Grid item xs={8} sm={10}>
                    <MultiFilePreview
                      key={service?.currentStage?.id}
                      thumbnail
                      files={mappedDisplayFiles.find((mappedFile) => mappedFile.stageId === service?.currentStage?.id)?.files || []}
        ...
        {(service && loadedServiceStages && loadedServiceStages.length > 0) && (
          <Box
            gap={2}
            display="grid"
            gridTemplateColumns={{
          ...
          >
            {mappedDisplayFiles
              .filter((mappedFile) => mappedFile.stageId !== service?.currentStage?.id)
              .map((mappedFile) => (
                <Paper
