...
export function MeasurementEditAttachments({
  measurement,
  refetchMeasurement,
  id,
  name,
  type,
  loadedMeasurementStages,
  isMobile,
  listPermissions,
}) {
  ...
  const mappedDisplayFiles = useMemo(() => {
    if (!loadedMeasurementStages || !measurement || !measurement.currentStage) return [];
    return loadedMeasurementStages?.map((stage) => {
      const filesForStage = displayFiles.filter((file) =>
        file.isNew ? measurement.currentStage.id === stage.id : file.current_stage && file.current_stage.id === stage.id
      );
      return {
  ...
  }, [displayFiles, loadedMeasurementStages, measurement]);
  ...
  useEffect(() => {
    const attachments = measurement.measurementAttachments || [];
        ...
        attachments.map(async (attachment) => {
          ...
          try {
            const response = await fetch(
              `${CONFIG.apiUrl}/measurements/get-file-url/?key=${encodeURIComponent(attachment.file)}`
            );
            if (!response.ok) {
  ...
  const handleConfirmRemove = async () => {
    ...
    if (!fileToRemove.isNew && !isLocalFile) {
      try {
        await axios.delete(`${CONFIG.apiUrl}/measurements/delete/file/${id}/measurement/${fileToRemove.file}/`, {
          data: {
  ...
  const handleAddFiles = async () => {
    ...
    try {
      ...
      filesToUpload.forEach((file) => {
        formData.append('measurementAttachments', file);
      });
      formData.append('userReporter', JSON.stringify(userLogged?.data));

      await axios.post(`${CONFIG.apiUrl}/measurements/upload/measurement/${id}/file/`, formData, {
        headers: {
  ...
  return (
    <>
      <Box sx={{ maxHeight: 600, minHeight: !isMobile ? 600 : 0, overflow: 'auto' }}>
        {(measurement?.currentStage &&
          measurement?.currentStage?.name.toLowerCase().indexOf(CONFIG.stages.finished.toLowerCase()) === -1) && (
            <>
              <Box
                key={measurement?.currentStage?.id}
                ...
                <Grid container spacing={2}>
                  <Grid item xs={4} sm={2}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                      <Box sx={{ mb: 1, typography: 'overline' }}>
                        <b>{measurement?.currentStage?.name}</b>
                      </Box>
                      {(verifyPermissions(
                        listPermissions,
                        CONFIG.permissions.system,
                        CONFIG.permissions.moduleMeasurements,
                  ...
                  <Grid item xs={8} sm={10}>
                    <MultiFilePreview
                      key={measurement?.currentStage?.id}
                      thumbnail
                      files={mappedDisplayFiles.find((mappedFile) => mappedFile.stageId === measurement?.currentStage?.id)?.files || []}
        ...
        {(measurement && loadedMeasurementStages && loadedMeasurementStages.length > 0) && (
          <Box
            gap={2}
            display="grid"
            gridTemplateColumns={{
          ...
          >
            {mappedDisplayFiles
              .filter((mappedFile) => mappedFile.stageId !== measurement?.currentStage?.id)
              .map((mappedFile) => (
                <Paper
