...
import { MeasurementDetailsCommentInput } from './measurement-details-comment-input';
import { MeasurementDetailsCommentListTrackInfo } from './measurement-details-comment-list-track-info';

// ----------------------------------------------------------------------

export function MeasurementDetailsCommentList({
  comments,
  measurement,
  refetchMeasurement,
  listSelectedTracks,
  ...
  useEffect(() => {
    if (comments && refetchMeasurement) {
      refetchMeasurement?.();
  ...
  }, [comments, refetchMeasurement]);

  useEffect(() => {
    if (listSelectedTracks && refetchMeasurement) {
      refetchMeasurement?.();
  ...
  }, [listSelectedTracks, refetchMeasurement]);
  ...
  const onDeleteComment = useCallback(
    async () => {
      ...
      try {
        const url = `${CONFIG.apiUrl}/measurements/delete/measurement/${measurement.id}/comment/${selectedComment.id}/`;
    ...
    }, [confirm, selectedComment, measurement.id, userLogged, loadedUsers]);
  ...
  return (
    <>
          ...
          comment.type === 'comment' ? (
            ...
            <Stack component="li" key={comment.id} direction="row" spacing={1} sx={{ bgcolor: 'transparent', borderRadius: 1, p: 1 }}>
              ...
              <Stack spacing={0.5} flexGrow={1}>
                ...
                {comment.managedData && (
                  <MeasurementDetailsCommentListTrackInfo comment={comment} />
                )}
              </Stack>
            </Stack>
          )
      ...
      <ConfirmDialog
        ...
        maxWidth="lg"
        content={
          <MeasurementDetailsCommentInput measurement={measurement} refetchMeasurement={refetchMeasurement} commentData={selectedComment} confirmEdit={confirmEdit} />
        }
      />
