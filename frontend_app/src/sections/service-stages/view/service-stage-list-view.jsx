...
import { ServiceStageTableRow } from '../service-stage-table-row';
import { ServiceStageTableToolbar } from '../service-stage-table-toolbar';
import { ServiceStageTableFiltersResult } from '../service-stage-table-filters-result';
...
export function ServiceStageListView() {

  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedServiceStages, loadingServiceStages, errorServiceStages, refetchServiceStages } = useDataContext();
  ...
  useEffect(() => {
    if (refetchServiceStages) {
      refetchServiceStages();
    }
    setTableData(loadedServiceStages || []);
  }, [refetchServiceStages, loadedServiceStages]);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await axios.delete(`${CONFIG.apiUrl}/services/delete/stage/${id}/`, {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await axios.delete(`${CONFIG.apiUrl}/services/delete/stages/`, {
        data: {
          ids: table.selected,
          userReporter: userLogged?.data,
        },
      ...
      console.error(error);
      toast.error('Error deleting the service stages');
    }
  ...
  const handleEditRow = useCallback(
    (id) => {
      ...
      router.push(paths.dashboard.serviceStage.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      ...
      router.push(paths.dashboard.serviceStage.details(id));
    },
    [router, filters]
  );

  if (loadingServiceStages) {
    return (
      <DashboardContent>
  ...
  if (errorServiceStages) {
    return (
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorServiceStages.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Service Stage', href: paths.dashboard.serviceStage.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.serviceStage.new}
              // color="inherit"
              // variant="outlined"
              variant="contained"
        ...
        <Card>
          ...
          <ServiceStageTableToolbar
            ...
            title={filters.state.status === 'all' ? 'All Service Stages' :
              filters.state.status === 'active' ? 'Active Service Stages' :
                filters.state.status === 'inactive' ? 'Inactive Service Stages' :
                  filters.state.status
            }
          />

          {canReset && (
            <ServiceStageTableFiltersResult
              filters={filters}
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> service stages?
          </>
        }
        action={
          <Button
