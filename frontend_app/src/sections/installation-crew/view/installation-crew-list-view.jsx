...
import { InstallationCrewTableRow } from '../installation-crew-table-row';
import { InstallationCrewTableToolbar } from '../installation-crew-table-toolbar';
import { InstallationCrewTableFiltersResult } from '../installation-crew-table-filters-result';
...
const STATUS_OPTIONS = [{ value: 'all', label: 'All Installation Crews' }].concat([
...
export function InstallationCrewListView() {

  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedInstallationCrews, loadingInstallationCrews, errorInstallationCrews, refetchInstallationCrews } = useDataContext();
  ...
  const TABLE_HEAD_MOBILE = [
    { id: 'info', label: 'Installation Crews' },
    { id: '' },
  ];
  ...
  const filters = useSetState({ name: '', status: localStorage.getItem('installationCrewStatus') || 'all' });
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentInstallationCrewId');
  }, []);
  ...
  useEffect(() => {
    if (refetchInstallationCrews) {
      refetchInstallationCrews();
    }
    setTableData(loadedInstallationCrews || []);
  }, [refetchInstallationCrews, loadedInstallationCrews]);

  useEffect(() => {
    if (loadedInstallationCrews) {
      setTableData(loadedInstallationCrews);
    }
  }, [loadedInstallationCrews]);

  useEffect(() => {
    const socket = new WebSocket(`${CONFIG.wsProtocol}://${CONFIG.wsHost}/${CONFIG.wsDomain}/projects/ws/project-installation-crews/`);
    // socket.onopen = () => {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await axios.delete(`${CONFIG.apiUrl}/projects/delete/installation-crew/${id}/`, {
          data: {
        ...
        console.error(error);
        toast.error('Error deleting the installation crew');
      }
    },
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await axios.delete(`${CONFIG.apiUrl}/projects/delete/installation-crews/`, {
        data: {
          ids: table.selected,
          userReporter: userLogged?.data,
        },
      ...
      console.error(error);
      toast.error('Error deleting the installation crews');
    }
  }, [dataFiltered.length, dataInPage.length, table, tableData, userLogged?.data]);

  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentInstallationCrewId', id);
      router.push(paths.dashboard.installationCrew.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      ...
      localStorage.removeItem('routeByShipment');
      localStorage.removeItem('routeByShipmentBySku');
      localStorage.setItem('installationCrewStatus', filters.state.status);
      router.push(paths.dashboard.installationCrew.details(id));
    },
    [router, filters]
  );

  if (loadingInstallationCrews) {
    return (
      <DashboardContent>
  ...
  if (errorInstallationCrews) {
    return (
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorInstallationCrews.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Installation Crew', href: paths.dashboard.installationCrew.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.installationCrew.new}
              // color="inherit"
              // variant="outlined"
              variant="contained"
              startIcon={<Iconify icon="mingcute:add-line" />}
            >
              New Installation Crew
            </Button>
          }
          sx={{ mb: { xs: 3, md: 5 } }}
        />
        ...
        <Card>
          ...
          <InstallationCrewTableToolbar
            ...
            title={filters.state.status === 'all' ? 'All Installation Crews' :
              filters.state.status === 'active' ? 'Active Installation Crews' :
                filters.state.status === 'inactive' ? 'Inactive Installation Crews' :
                  filters.state.status
            }
          />

          {canReset && (
            <InstallationCrewTableFiltersResult
              filters={filters}
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> installation crews?
          </>
        }
        action={
          <Button
