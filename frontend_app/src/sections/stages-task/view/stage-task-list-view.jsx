...
import { StageTaskTableFiltersResult } from '../stage-task-table-filters-result';
...
export function StageTaskListView() {

  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedStagesTask, loadingStagesTask, errorStagesTask, refetchStagesTask } = useDataContext();
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentStageTaskId');
  }, []);
  ...
  useEffect(() => {
    if (refetchStagesTask) {
      refetchStagesTask();
    }
    setTableData(loadedStagesTask || []);
  }, [refetchStagesTask, loadedStagesTask]);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await axios.delete(`${CONFIG.apiUrl}/projects/delete/stage-task/${id}/`, {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await axios.delete(`${CONFIG.apiUrl}/projects/delete/stages-task/`, {
        data: {
          ids: table.selected,
          userReporter: userLogged?.data,
        },
  ...
  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentStageTaskId', id);
      router.push(paths.dashboard.stageTask.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      ...
      router.push(paths.dashboard.stageTask.details(id));
    },
    [router, filters]
  );

  if (loadingStagesTask) {
    return (
      <DashboardContent>
      ...
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorStagesTask.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Task Stage', href: paths.dashboard.stageTask.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.stageTask.new}
              // color="inherit"
              // variant="outlined"
              variant="contained"
