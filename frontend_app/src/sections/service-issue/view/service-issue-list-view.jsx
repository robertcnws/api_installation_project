...
import { ServiceIssueTableRow } from '../service-issue-table-row';
import { ServiceIssueTableToolbar } from '../service-issue-table-toolbar';
import { ServiceIssueTableFiltersResult } from '../service-issue-table-filters-result';
...
const STATUS_OPTIONS = [{ value: 'all', label: 'All Service Issues' }].concat([
...
export function ServiceIssueListView() {

  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedServiceIssues, loadingServiceIssues, errorServiceIssues, refetchServiceIssues } = useDataContext();
  ...
  const filters = useSetState({ name: '', status: localStorage.getItem('serviceIssueStatus') || 'all' });
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentServiceIssueId');
  }, []);
  ...
  useEffect(() => {
    if (refetchServiceIssues) {
      refetchServiceIssues();
    }
    setTableData(loadedServiceIssues || []);
  }, [refetchServiceIssues, loadedServiceIssues]);

  useEffect(() => {
    if (loadedServiceIssues) {
      setTableData(loadedServiceIssues);
    }
  }, [loadedServiceIssues]);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await axios.delete(`${CONFIG.apiUrl}/services/delete/service-issue/${id}/`, {
          data: {
        ...
        console.error(error);
        toast.error('Error deleting the service issue');
      }
    },
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await axios.delete(`${CONFIG.apiUrl}/services/delete/service-issues/`, {
        data: {
          ids: table.selected,
          userReporter: userLogged?.data,
        },
      ...
      console.error(error);
      toast.error('Error deleting the service issues');
    }
  }, [dataFiltered.length, dataInPage.length, table, tableData, userLogged?.data]);

  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentServiceIssueId', id);
      router.push(paths.dashboard.serviceIssue.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      ...
      localStorage.removeItem('routeByShipment');
      localStorage.removeItem('routeByShipmentBySku');
      localStorage.setItem('serviceIssueStatus', filters.state.status);
      router.push(paths.dashboard.serviceIssue.details(id));
    },
    [router, filters]
  );

  if (loadingServiceIssues) {
    return (
      <DashboardContent>
  ...
  if (errorServiceIssues) {
    return (
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorServiceIssues.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Service Issue', href: paths.dashboard.serviceIssue.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.serviceIssue.new}
              // color="inherit"
              // variant="outlined"
              variant="contained"
        ...
        <Card>
          ...
          <ServiceIssueTableToolbar
            ...
            title={filters.state.status === 'all' ? 'All Service Issues' :
              filters.state.status === 'active' ? 'Active Service Issues' :
                filters.state.status === 'inactive' ? 'Inactive Service Issues' :
                  filters.state.status
            }
          />

          {canReset && (
            <ServiceIssueTableFiltersResult
              filters={filters}
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> service issues?
          </>
        }
        action={
          <Button
