...
export function useFilteredProjects(projects, loadedUsers, userLogged) {
  ...
  if (!roles.some(r => [CONFIG.roles.financialStaff, CONFIG.roles.warehouseStaff].includes(r))) {
    ...
    if (userLogged.data.user_role.name.toLowerCase().includes(CONFIG.roles.installer.toLowerCase())) {
      finalProjects = sortedProjects.filter(p => (
        (p.currentStage.name.toLowerCase().includes(CONFIG.stages.installation.toLowerCase()) && p.usersAssignees.some(u => u.username === userLogged.data.username)) ||
        (p.currentStage.name.toLowerCase().includes(CONFIG.stages.coordination.toLowerCase()) && totalPercentageProjectStage(p, CONFIG.stages.coordination, CONFIG) >= 50 && p.usersAssignees.some(u => u.username === userLogged.data.username))
      ));
    }
  }
